...
struct file_elem {
  struct list_elem elem;     // 리스트에 들어갈 때 필수
  int fd;                    // 파일 디스크립터 번호
  struct file *file;         // 실제 파일 포인터
};
...
static struct file *get_file_by_fd(int fd) {
  ...
  for (e = list_begin(&cur->file_table); e != list_end(&cur->file_table); e = list_next(e)) {
...
remove_from_file_table (int fd)
{
  ...
  for (e = list_begin (&t->file_table); e != list_end (&t->file_table); e = list_next (e))
    {
      struct file_elem *entry = list_entry (e, struct file_elem, elem); // ✅ 타입 수정
      if (entry->fd == fd)
        {
...
static int add_to_file_table(struct file *f) {
  ...
  list_push_back(&cur->file_table, &fe->elem); // 💥 오류 나던 부분 여기 고침
