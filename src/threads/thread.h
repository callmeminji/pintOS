...
/* States in a thread's life cycle. 스레드 상태 정의*/
enum thread_status
  {
...
struct thread
  {
    /* Owned by thread.c. */
    tid_t tid;                          /* Thread identifier. 스레드 고유 번호 */
    enum thread_status status;          /* Thread state. 스레드 현재 상태 */
    char name[16];                      /* Name (for debugging purposes). 디버깅용 이름*/
    uint8_t *stack;                     /* Saved stack pointer.우선 순위위 */
...
void refresh_priority(void);// 함수 추가
