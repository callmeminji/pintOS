...
/* List of sleeping threads -잠든 스레드들 저장할 리스트 */
...
thread_unblock (struct thread *t) 
{
 ...
 list_insert_ordered (&ready_list, &t->elem, compare_priority, NULL); //리스트에 들어가는 순서 자동 조정, priority 기준으로
...
void thread_sleep(int64_t wake_up_time) {
  enum intr_level old_level = intr_disable();  // 인터럽트 잠깐 끔

  struct thread *cur = thread_current();       // 현재 스레드
  cur->wake_up_time = wake_up_time;            // 언제 깰지 저장
  list_push_back(&sleep_list, &cur->elem);     // sleep_list에 넣기
  thread_block();                              // 스레드 block 상태로 만들기

  intr_set_level(old_level);                   // 인터럽트 복원
}

/* 시간이 wake_up_time 이상인 스레드를 깨우는 함수*/
